...
train_loss, val_loss = nn.fit(X_train, y_train, 32, 0.01, X_cv, y_cv, stop_train=5, brk=5000) # model may run for a very long time, please put "brk = 1000" to break after 1000 steps
pred = nn.predict(X_cv)

plt.plot(train_loss)
plt.title('Train Loss for Model 3')
plt.savefig('Model3_Train_loss.png')
plt.show()
plt.plot(val_loss)
plt.title('Validation Loss for Model 3')
plt.savefig('Model3_val_loss.png')
plt.show()
...
with open('Model3_weights.w', 'wb') as file:
